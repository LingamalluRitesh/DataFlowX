"""
DataFlowX Oracle SQL Dialect Generator
Generates Oracle 19c/21c SQL queries from AST nodes with FETCH FIRST N ROWS ONLY, double-quoted uppercase identifiers, and NVL.
"""
...
class OracleDialectGenerator:
    """Generates Oracle SQL from SelectQueryAST."""
    ...
    def generate(cls, ast: SelectQueryAST) -> str:
        ...
        for p in ast.projections:
            ...
            if p.alias:
                proj_strs.append(f'{expr_str} "{p.alias.upper()}"')
            else:
        ...
        if ast.limit is not None:
            sql += f" FETCH FIRST {ast.limit} ROWS ONLY"
    ...
    def _format_table(cls, tbl: TableRefNode) -> str:
        t = f'"{tbl.schema_name.upper()}"."{tbl.table_name.upper()}"' if tbl.schema_name else f'"{tbl.table_name.upper()}"'
        if tbl.alias:
            t += f' "{tbl.alias.upper()}"'
        return t
